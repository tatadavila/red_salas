<template>
  <div class="app__question" v-if="!loader && start && !result">
    <div class="app__question-header">
      <span class="app__question-counter"
        >{{ totalAnswers + 1 }}/{{ questionAmount }}</span
      >
      <h3 class="app__question-title" v-html="question.question"></h3>
      <p
        class="app__question-caption"
    ...
    <button
      class="app__btn btn btn--bold btn--center"
      v-on:click="checkAnswer(question)"
    >
      Next
    </button>
  </div>
